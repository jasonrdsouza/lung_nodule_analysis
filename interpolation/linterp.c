...
float      xd, yd, zd, x1, x2, y1, y2, w1, w2, p; //temp variables for linear interp
    ...
    VXmake3dim(&l_interp, VX_PBYTE, bbx, 1);
    ...
    for (k = l_interp.zlo; k <= l_interp.zhi; k++){
      for (j = l_interp.ylo; j <= l_interp.yhi; j++){
        for (i = l_interp.xlo; i <= l_interp.xhi; i++){
	    /* remap from the new voxel space to the new voxel space */
            xt= i;
            yt= j;
            zt= k;
            ...
            // 8 NN pixels
            orig.u[nnzlo][nnylo][nnxlo];
            orig.u[nnzlo][nnylo][nnxhi];
            orig.u[nnzlo][nnyhi][nnxlo];
            orig.u[nnzlo][nnyhi][nnxhi];

            orig.u[nnzhi][nnylo][nnxlo];
            orig.u[nnzhi][nnylo][nnxhi];
            orig.u[nnzhi][nnyhi][nnxlo];
            orig.u[nnzhi][nnyhi][nnxhi];

            // interpolation caclculation
	    ...
	    x1= orig.u[nnzlo][nnylo][nnxlo] * (1-zd) + orig.u[nnzhi][nnylo][nnxlo] * zd;
	    x2= orig.u[nnzlo][nnyhi][nnxlo] * (1-zd) + orig.u[nnzhi][nnyhi][nnxlo] * zd;
	    y1= orig.u[nnzlo][nnylo][nnxhi] * (1-zd) + orig.u[nnzhi][nnylo][nnxhi] * zd;
	    y2= orig.u[nnzlo][nnyhi][nnxhi] * (1-zd) + orig.u[nnzhi][nnyhi][nnxhi] * zd;
            ...
            // final interpolation
            l_interp.u[zt][yt][xt]= w1 * (1-xd) + w2 * xd;
	}
      }
    }
